...
$usuariosnuevos_consulta= "SELECT id, idnumber, email FROM mdl_user WHERE idnumber>='0' ORDER BY id ASC LIMIT 3";
...
while ($var_fila=$usuariosnuevos_resultado->fetch_array()) {
	...
	$correoibero='';
	$correoibero_consulta = "SELECT id, email FROM mdl_user_claves_old WHERE idnumber='".$idnumber."'";
	echo $correoibero_consulta ;
	
	$correoibero_resultado = $obj_conexion->query($correoibero_consulta );
	while ($clave_fila=$correoibero_resultado->fetch_array()) {
		$correoibero = $clave_fila["email"];
	}
	echo $idusuario . " | Correo:" . $correoibero;
	//exit();
	if ($correoibero=='') {
		echo "No se cambio el correo actualizar el registro. <br>";
	} else {
